...
class Json_Rpc_Client_Tests(unittest.TestCase):
    ...
    def test_send_multiple_request(self):
        ...
        # Minimum sleep time for main thread so background threads can process the requests.
        time.sleep(1)
    ...
    def test_receive_invalid_response_exception(self):
        ...
        try:
            ...
            self.assertEqual(exception.args, ('Content-Length was not found in headers received.',))
            # Expect a look up error exception since the content-length is not found
            self.assertTrue(test_client.request_thread.isAlive())
            self.assertFalse(test_client.response_thread.isAlive())
    ...
    def test_response_stream_closed_exception(self):
        ...
        try:
            ...
            # Expecting a Value error as we are attempting to access a closed stream.
            self.assertTrue(test_client.request_thread.isAlive())
            self.assertFalse(test_client.response_thread.isAlive())
            self.assertEqual(threading.active_count(), 2)      
            test_client.shutdown()
    ...
    def test_stream_closed_during_process(self):
        ...
        # Minimum sleep time to give the request thread time to process the request.
        time.sleep(1)
